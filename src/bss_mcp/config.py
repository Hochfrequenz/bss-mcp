...
def get_bss_client(settings: BssMcpSettings) -> BssClient:
    server_url = URL(settings.url)
    if settings.auth_type == AuthType.BASIC:
        assert settings.user and settings.password, "BSS_USER and BSS_PASSWORD required for basic auth"
        return BasicAuthBssClient(
            BasicAuthBssConfig(server_url=server_url, usr=settings.user, pwd=settings.password)
        )
    assert settings.client_id and settings.client_secret and settings.token_url, (
        "BSS_CLIENT_ID, BSS_CLIENT_SECRET, BSS_TOKEN_URL required for OAuth"
    )
